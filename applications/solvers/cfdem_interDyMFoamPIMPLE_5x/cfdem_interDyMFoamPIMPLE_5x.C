...
int main(int argc, char *argv[])
{
	...
	if (runTime.elapsedClockTime() > maxSimTime || alphaAvg >= maxAlpha1 || alphaAvg <=minAlpha1  )
	{
	    Info<< "\n\nexecutionTime = " << runTime.elapsedCpuTime() << " s, "
                    << "  S1 = average(alpha) = " << alphaAvg << "  \n"
		    << "  maxExecutionTime ("<<maxSimTime<<"),   "
                    << "  maxS1 ("<<maxAlpha1<<") or   "
                    << "  minS1 ("<<minAlpha1 <<") reached, ending simulation. "
                    << "\n  Re-set above keywords in file controlDict to continue. "
                    << endl;
